...
def handle_telegram_message(update: Update, context: CallbackContext):
    ...
    if command:
        ...
        try:
            ...
            stdin, stdout, stderr = ssh.exec_command(f'test -f /opt/script/update-node-yml/{script_name.split(".")[0]}.sh && echo "exists" || echo "not exists"')
...
def button_callback(update: Update, context: CallbackContext):
    ...
    if button_data == 'update_json':
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        query.edit_message_text("⚠️ Are you sure you want to execute the <b>update_json</b> script?", reply_markup=reply_markup, parse_mode=ParseMode.HTML)
    else:
    ...
    try:
        ...
        if file_check_output == "not exists":
            ...
            if 'No such file or directory' in error_output:
                query.edit_message_text(f"❌ **Error**: Script '{button_data}.sh' not found on the remote server.", parse_mode=ParseMode.MARKDOWN_V2)
            else:
                chunks = split_message(output)
                for chunk in chunks:
...
def confirm_update_json_callback(update: Update, context: CallbackContext):
    ...
    if button_data == 'confirm_update_json':
        ...
        query.edit_message_text("⛔ The update_json script execution was canceled.", parse_mode=ParseMode.HTML)
